	...
	void OnGUI ()
	{
		...
		if (GUI.Button(new Rect((Screen.width - btnGap) / 2 - btnWidth, btnTop, btnWidth, btnHeight), "Show Share Menu"))
		{
            ...
            content.SetEnableClientShare(true);
                ...
                //通过分享菜单分享
                ssdk.ShowPlatformList (new [] { PlatformType.WeChat}, content, 100, 100);
		...
		btnTop += btnHeight + 20 * scale;
		if (GUI.Button(new Rect((Screen.width - btnGap) / 2 - btnWidth, btnTop, btnWidth, btnHeight), "Share WeChat"))
		{
#if UNITY_IPHONE
            ShareContent content = new ShareContent ();
